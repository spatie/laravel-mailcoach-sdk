...
```bash
php artisan vendor:publish --tag="laravel-mailcoach-sdk-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="laravel-mailcoach-sdk-config"
```
...
```bash
php artisan vendor:publish --tag="laravel-mailcoach-sdk-views"
```

## Usage

```php
$mailcoachSdk = new Spatie\MailcoachSdk();
echo $mailcoachSdk->echoPhrase('Hello, Spatie!');
```
